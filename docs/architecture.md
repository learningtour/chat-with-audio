...
```
Claude (chat) ── MCP (stdio) ──> server.py (33 tools, FastMCP)
                                     │
             ┌───────────────────────┼────────────────────────┐
             ▼                       ▼                        ▼
        analysis.py             chain.py                sessions.py
        compliance.py           STEP_REGISTRY           timeline/compliance
        regions.py              33 steps                log.md provenance
        segments.py                 │                        │
             │                      ▼                        ▼
...
`chain.py` holds `STEP_REGISTRY` (33 steps) and `run_chain`, which validates
